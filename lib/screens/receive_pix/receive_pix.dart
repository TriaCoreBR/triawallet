...
import 'package:triacore_mobile/providers/triacore/triacore_client/user_provider.dart';
import 'package:triacore_mobile/providers/wallet/liquid_provider.dart';
import 'package:triacore_mobile/screens/receive_pix/generate_pix_payment_code.dart';
...
class ReceivePixScreen extends ConsumerStatefulWidget {
  ...
  const ReceivePixScreen({Key? key}) 
    : initialAmount = null,
      super(key: key);
      
  const ReceivePixScreen.withAmount({
    Key? key,
...
class ReceivePixState extends ConsumerState<ReceivePixScreen> {
  ...
  void initState() {
    ...
    // Definir o valor inicial se fornecido
    if (widget.initialAmount != null && widget.initialAmount! > 0) {
      final initialAmount = widget.initialAmount!;
      amountController.text = initialAmount.toStringAsFixed(2);
      _currentAmountFloat = initialAmount;
      _currentAmountInCents = (initialAmount * 100).round();
    }
