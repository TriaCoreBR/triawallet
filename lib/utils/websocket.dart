...
/// Evento de erro para facilitar tratamento visual na UI
class WebSocketErrorEvent {
  final String message;
  final dynamic error;
  WebSocketErrorEvent(this.message, [this.error]);
}
    ...
    try {
      ...
      _channel!.stream.listen(
        ...
        onError: (error) {
          ...
          // Emite evento de erro no stream
          if (!_isDisposed && !_controller.isClosed) {
            _controller.add(WebSocketErrorEvent('Erro de conexão WebSocket', error));
          }
        },
        onDone: () {
      ...
      await _connectionCompleter?.future.timeout(
        ...
        onTimeout: () {
          // Emite evento de timeout no stream
          if (!_isDisposed && !_controller.isClosed) {
            _controller.add(WebSocketErrorEvent('Tempo de conexão esgotado (timeout)'));
          }
      ...
      _handleDisconnect();
      // Emite evento de erro genérico no stream
      if (!_isDisposed && !_controller.isClosed) {
        _controller.add(WebSocketErrorEvent('Erro ao conectar ao WebSocket', e));
      }
      rethrow;
    } finally {
